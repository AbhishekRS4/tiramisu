...
from tiramisu_utils import init, read_config_file, get_tf_dataset
import tiramisu_model
...
def compute_metrics(groundtruth, prediction, axis = 1, num_classes = 15):
    ...
    with tf.name_scope('valid_metrics'):
        ...
        iou_value, iou_op = tf.metrics.mean_iou(groundtruth, predicted_labels, num_classes = num_classes)
...
def compute_loss(groundtruth, prediction, axis = 1, name = 'mean_cross_entropy'):
    ...
    mean_ce = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels = groundtruth, logits = prediction), name = name)
...
def get_optimizer(initial_learning_rate, loss_function, global_step, epsilon = 0.0001):
    ...
    learning_rate = tf.train.polynomial_decay(initial_learning_rate, global_step, decay_steps, end_learning_rate, power = power)
    adam_optimizer_op = tf.train.AdamOptimizer(learning_rate = learning_rate, epsilon = epsilon).minimize(loss_function)
...
def batch_train(FLAGS):
    ...
    list_labels_train = [os.path.join(FLAGS.labels_dir_train, x.split('.')[0] + '_L.png') for x in list_train]

    # list of validation images and labels
    list_images_valid = [os.path.join(FLAGS.images_dir_valid, x) for x in list_valid]
    list_labels_valid = [os.path.join(FLAGS.labels_dir_valid, x.split('.')[0] + '_L.png') for x in list_valid]
    ...
    dataset_train = get_tf_dataset(list_images_train, list_labels_train, FLAGS.num_epochs, FLAGS.batch_size)
    dataset_valid = get_tf_dataset(list_images_valid, list_labels_valid, FLAGS.num_epochs, FLAGS.batch_size)
    ...
    net_arch.tiramisu_net(features)
    logits = net_arch.logits
    print('tiramisu-' + str(FLAGS.model_to_use) + ' model built')
    ...
    acc_value, acc_op, iou_value, iou_op = compute_metrics(labels, logits, axis = axis, num_classes = FLAGS.num_classes)
    ...
    for epoch in range(FLAGS.num_epochs):
        ...
        for batch_id in range(num_batches_train):
            #_, _, loss_per_batch = ss.run([extra_update_op, optimizer_op, loss], feed_dict = {is_training : True, global_step : epoch})
            _, loss_per_batch = ss.run([optimizer_op, loss], feed_dict = {is_training : True, global_step : epoch})
            temp_train_loss_per_epoch += loss_per_batch

        ss.run(init_op_valid)
        for batch_id in range(num_batches_valid):
            loss_per_batch, _, _ = ss.run([loss_1, acc_op, iou_op], feed_dict = {is_training : False})
        ...
        print('Epoch : {0:d} / {1:d}, time taken : {2:.2f} sec.'.format(epoch + 1, FLAGS.num_epochs, ti))
        print('training loss : {0:.4f}, validation loss : {1:.4f}, validation accuracy : {2:.4f}, validation mean iou : {3:.4f}'.format(temp_train_loss_per_epoch / num_batches_train, temp_valid_loss_per_epoch / num_batches_valid, acc_valid, iou_valid))
        print('')
...
def main():
    ...
    parser.add_argument('-images_dir_train', default = images_dir_train, type = str, help = 'directory with training images')
    parser.add_argument('-labels_dir_train', default = labels_dir_train, type = str, help = 'directory with training labels')
    parser.add_argument('-images_dir_valid', default = images_dir_valid, type = str, help = 'directory with validation images')
    parser.add_argument('-labels_dir_valid', default = labels_dir_valid, type = str, help = 'directory with validation labels')
    ...
    parser.add_argument('-batch_size', default = batch_size, type = int, help = 'number of samples in a batch')
    parser.add_argument('-weight_decay', default = weight_decay, type = float, help = 'weight decay')
    parser.add_argument('-checkpoint_epoch', default = checkpoint_epoch, type = int, help = 'checkpoint epoch to save every kth model')

    parser.add_argument('-model_dir', default = model_dir, type = str, help = 'directory to save the model')
    parser.add_argument('-model_file', default = model_file, type = str, help = 'file name to save the model')
    parser.add_argument('-model_metrics', default = model_metrics, type = str, help = 'file name to save metrics')
