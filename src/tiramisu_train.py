...
def compute_loss(groundtruth, prediction, axis=1, name="mean_cross_entropy"):
    ...
    mean_ce = tf.reduce_mean(
        tf.nn.sparse_softmax_cross_entropy_with_logits(labels=groundtruth, logits=prediction), name=name
    )
...
def batch_train(FLAGS):
    ...
    tira_net = Tiramisu(is_training, FLAGS.data_format, FLAGS.num_classes, FLAGS.model_to_use)
    tira_net.tiramisu_net(features)
    logits = tira_net.logits
    print(f"tiramisu-{FLAGS.model_to_use} model built")
    ...
    for epoch in range(FLAGS.num_epochs):
        ...
        for batch_id in range(num_batches_train):
            ...
            _, loss_per_batch = ss.run(
                [optimizer_op, loss], feed_dict={is_training: True, global_step: epoch}
            )
        ...
        print(f"training loss : {temp_train_loss_per_epoch / num_batches_train}, \
            ...
            validation accuracy : {acc_valid}, validation mean iou : {iou_valid}")
